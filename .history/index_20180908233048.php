...
if(isset($_POST['submit_login'])){
    ...
    if(!empty($_POST['UserName']) && !empty($_POST['UserPass'])){
        ...
        $sql = "SELECT * FROM users WHERE UserName = '".$post_UserName."' AND UserPass = '".$post_UserPass."'";
        if($result = mysqli_query($conn, $sql)){
            while($rows = mysqli_fetch_assoc($result)){
                if(mysqli_num_rows($result) == 1){
                    ...
                    setcookie('datosUsuarios', {'user': $post_UserName, 'password': $post_UserPass, 'role': $rows['role']}, time() + 3600);
