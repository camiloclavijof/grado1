...
if(isset($_POST['UserEmail'])){
    ...
    if($result = mysqli_query($conn, $sql)){
        ...
        while($rows = mysqli_fetch_assoc($result)){
            $post_UserEmail = mysqli_real_escape_string($conn,$rows['id_users']);
            $sql2 = 'DELETE FROM users WHERE id_users = '.$rows['id_users'].';';
            if($result2 = mysqli_query($sql2)){
                while($rows2 = mysqli_fetch_assoc($result2)){
                    alert('Usuario '.$rows['UserName'].' Eliminado.');
                }
            }
        }
    }
